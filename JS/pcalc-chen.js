...
(function(g){
  ...
  PCALC.chenPercent=s=>Math.max(0,Math.min(100,(s/20)*100));
  PCALC.preflopLabel=p=>p>=85?'Premium':p>=70?'Forte':p>=55?'Marginal':'Fraca';

  PCALC.renderPreflopPanel=function(){
    const box=document.getElementById('preflopBox');
    if(!box) return;
    const {hand,board}=PCALC.getKnown();
    if(hand.length===2 && board.length<3){
      const cs=PCALC.chenScore(hand[0],hand[1]);
      const pct=PCALC.chenPercent(cs.score);
      const lab=PCALC.preflopLabel(pct);
