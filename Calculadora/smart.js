(() => {
  ...
  function run(rb) {
    ...
    function getPotOddsContainer() {
      return q('#pcalc-sugestao .raise-potodds.card', rb);
    }

    function readBE() {
      return byLabelNextB(getPotOddsContainer(), 'BE (pot odds)');
    }

    function readEquityMC() {
      return byLabelNextB(getPotOddsContainer(), 'Equity (MC)');
    }

    function readPot() {
      return byLabelNextB(getPotOddsContainer(), 'Pot (fichas)');
    }

    function readToCall() {
      return byLabelNextB(getPotOddsContainer(), 'A pagar (fichas)');
    }
    ...
    function readBoard() {
      return readByIds(['#b0', '#b1', '#b2', '#b3', '#b4'], rb);
    }
    ...
    function boardWetness(cards, heroCards) {
      ...
      for (const c of cards)
        if (suitCount[c.suit] != null) suitCount[c.suit]++;
      const flushMax = Math.max(suitCount.c, suitCount.d, suitCount.h, suitCount.s);

      let toneLabel, toneF;
      if (flushMax >= 5) {
        toneLabel = 'flush completo no board';
        toneF = 0.65;
      } else if (flushMax === 4) {
        toneLabel = 'flush draw extremo';
        toneF = 0.72;
      } else if (flushMax === 3) {
        toneLabel = 'flush draw forte';
        toneF = 0.82;
      } else if (flushMax === 2) {
        toneLabel = 'flush draw';
        toneF = 0.92;
      } else {
        toneLabel = 'board seco';
        toneF = 1.00;
      }
      ...
      let connLabel, connF;
      if (hasSeq3) {
        connLabel = 'straight draw forte';
        connF = 0.80;
      } else if (hasNearSeq) {
        connLabel = 'straight draw';
        connF = 0.88;
      } else {
        let cons = false,
          near = false;
        ...
        if (cons || near) {
          connLabel = 'semi conectado';
          connF = 0.94;
        } else {
          connLabel = 'desconectado';
          connF = 1.00;
        }
      }
      ...
      if (hasQuads) {
        extras.push('quadra no board');
        pairF *= 0.72;
      }
      if (hasFull) {
        extras.push('full house no board');
        pairF *= 0.75;
      } else if (hasTrips) {
        extras.push('trinca no board');
        pairF *= 0.78;
      }
      if (hasTwoPair) {
        extras.push('dois pares no board');
        pairF *= 0.85;
      } else if (!hasTrips && hasPair) {
        extras.push('par no board');
        pairF *= 0.90;
      }
      ...
      if (overToBoard) {
        favorF *= 1.02;
        favorTags.push('overcards úteis');
      }
      ...
      for (let i = 0; i <= allRanksSorted.length - 4; i++) {
        if (allRanksSorted[i + 3] - allRanksSorted[i] <= 4) {
          seqSpan4 = true;
          break;
        }
      }
      if (seqSpan4) {
        favorF *= 1.02;
        favorTags.push('sequência bem próxima');
      }
    ...
    // ============ FUNÇÃO decide MODIFICADA (2ª vez) ============
    function decide(eSmart, be) {
      if (!isFinite(eSmart) || !isFinite(be)) {
        return { label: 'Sem dados suficientes', level: 'neutral' };
      }

      // --- REGRA 1: Piso de equity ---
      // "nao pagar valores onde meu score fla que nao esta bom e tenho muita chance de perder"
      if (eSmart < 30) {
        return { label: 'Passe ou Desista (Equity < 30%)', level: 'fold' };
      }

      // --- REGRA 2: Comparação com Pot Odds (BE) ---
      // "pague a aposta se tiver um bom score realista" (ou seja, eSmart > BE)
      
      if (eSmart >= be * 1.20) {
        // Muito lucrativo
        return { label: 'Aposte por valor (Equity Alta vs BE)', level: 'strong' };
      }
      if (eSmart >= be * 1.05) {
        // Lucrativo
        return { label: 'Pague a aposta (Equity > BE)', level: 'good' };
      }
      if (eSmart >= be * 0.95) {
        // Limítrofe / "Thin"
        return { label: 'Pague ou Desista (Equity ≈ BE)', level: 'thin' };
      }

      // --- REGRA 3: Não bate o BE ---
      // Se eSmart é >= 30%, mas não bate o BE (eSmart < be * 0.95)
      return { label: 'Passe ou Desista (Equity < BE)', level: 'fold' };
    }

    // --- Painel
    function mountPanel() {
      const ex = q('#smart-rec-panel', rb);
      if (ex) ex.remove();
...
<div>A pagar</div><div id="srp-call">—</div>
    ...
    function render(s) {
      ...
      if (lab) {
        lab.textContent = s.immune ?
          'Mão perfeita — pague qualquer valor ou aposte all-in' :
          rec.label;
      }
      paint(s.immune ? 'strong' : rec.level);

      const ok = (isFinite(s.eq) && isFinite(s.be) && s.be > 0);
      const st = q('#srp-status', rb);
      if (st) st.textContent = ok ? 'ok' : 'aguardando…';
    }
    ...
    function observe() {
      ...
      const potC = getPotOddsContainer();
      if (potC) obs.observe(potC, {
        childList: true,
        subtree: true,
        characterData: true
      });
      const eqb = q('#equityBox', rb) || q('#equityBox');
      if (eqb) obs.observe(eqb, {
        childList: true,
        subtree: true,
        characterData: true
      });
      const boardRow = q('.row', rb) || q('.row');
      if (boardRow) obs.observe(boardRow, {
        childList: true,
        subtree: true,
        characterData: true
      });
      ...
      const recalc = q('#btnEqCalc', eqb || rb);
      if (recalc) recalc.addEventListener('click', tick, {
        passive: true
      });
      ...
      return () => {
        obs.disconnect();
        clearInterval(id);
        sel && sel.removeEventListener('change', tick);
        recalc && recalc.removeEventListener('click', tick);
      };
    }
    ...
    (function start() {
      ...
      const stop = observe();
      tick();
      window.__smartRecRightKill = () => {
        try {
          stop();
        } catch (e) {};
        const n = q('#smart-rec-panel', rb);
        if (n) n.remove();
        console.log('[SmartRec] painel (direita) removido');
      };
