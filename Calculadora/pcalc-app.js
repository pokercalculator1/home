...
    // MODIFICADO: Mapeia TODOS os grupos, não apenas o top 5
    const allGroups = arr.map(g=>{
      return {
...
    return {
      allGroups, // MODIFICADO: Renomeado de 'top5' para 'allGroups'
      hero: {
        eval: heroEv,
...
  // MODIFICADO: Agora lista todos os grupos pós-flop
  function computeTop5PostflopLeaderboard(){
    const data = computePostflopLeaderboard();
    if(!data) return null;
    // MODIFICADO: Itera sobre 'allGroups', não 'top5'
    const rows = data.allGroups.map((it, idx)=>({
...
  // NOVO: Injeta CSS para a barra de rolagem personalizada do overlay
  function injectOverlayScrollbarStyles() {
    if (document.getElementById('pcalc-overlay-scroll-styles')) return;
    const style = document.createElement('style');
    style.id = 'pcalc-overlay-scroll-styles';
    style.textContent = `
      .nuts-overlay-list {
        max-height: 350px; /* Altura máxima antes de rolar */
        overflow-y: auto;
        padding-right: 5px; /* Espaço para a barra */
        /* Firefox */
        scrollbar-width: thin;
        scrollbar-color: #222 #0b1324; /* Cor da barra / Cor do fundo */
      }
      /* Chrome, Safari, Edge */
      .nuts-overlay-list::-webkit-scrollbar {
        width: 8px;
      }
...
        border-radius: 4px;
        border: 2px solid #0b1324; /* "padding" em volta da barra */
      }
    `;
...
    if(!anchor) return;
    hideNutsOverlay();
    injectOverlayScrollbarStyles(); // NOVO: Garante que o CSS da scrollbar existe
...
    title.textContent = isPreflop ? 'Top 5 (pré-flop, JSON) + sua posição' : 'Todas as mãos possíveis (board atual)'; // Título modificado
    wrap.appendChild(title);

    const list=document.createElement('div');
    list.className = 'nuts-overlay-list'; // NOVO: Aplica a classe da scrollbar
...
    }else{
      // MODIFICADO: Agora 'd' contém TODAS as linhas, não só o top 5
      const d = computeTop5PostflopLeaderboard();
      if(d && d.rows?.length){
        // Itera sobre TODAS as linhas
        d.rows.forEach((it, idx)=>{
          const row=document.createElement('div');
          // Permite quebra de linha para os status do herói
          row.style.cssText='display:flex;justify-content:space-between;gap:10px;padding:4px 0;flex-wrap:wrap;';
          
...
          // --- NOVO: Lógica para destacar a mão do Herói ---
          const isHeroRow = (idx + 1) === d.hero.classPosition;
          if (isHeroRow) {
            // ID para o scroll
            row.id = 'hero-row-in-overlay'; 
            // Estilo de destaque
            row.style.background = 'rgba(139, 92, 246, 0.15)'; // Fundo destacado
...
        /*           REMOVIDO: Bloco do herói separado no final.
          Agora ele é destacado diretamente na lista.
        */
...
T     const tie = (T/tot*100).toFixed(1);
