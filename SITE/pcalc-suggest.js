// pcalc-suggest.js
import { CAT } from './pcalc-core.js';
import { computeOuts } from './pcalc-outs.js';
import { chenScore, chenPercent } from './pcalc-chen.js';

let _lastDecisionClass = null;
export function decisionClass(title){
...
export function shouldGlow(cls){
  const glow = (_lastDecisionClass && _lastDecisionClass !== cls);
  _lastDecisionClass = cls;
  return glow;
}

export function suggestAction(eqPct, hand, board, opp){
...
export function eqPctPreflop(hand){
  const pct = chenPercent(chenScore(hand[0],hand[1]).score);
  return pct;
}
