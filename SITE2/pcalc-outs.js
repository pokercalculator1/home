// pcalc-outs.js
import { PCalcState, makeDeck, cardId, evalBest, CAT, CAT_NAME, SUIT_CLASS, SUIT_GLYPH, fmtRank } from './pcalc-core.js';
...
function computeByRiverCountsExact(hand, board, remaining){
  ...
  const n = remaining.length, totalPairs = n*(n-1)/2;
  const byRiverCounts = Object.fromEntries(targets.map(t=>[t,0]));
  for(let i=0;i<n-1;i++){
    for(let j=i+1;j<n;j++){
      const ev = evalBest(hand.concat(board,[remaining[i],remaining[j]]));
      if(byRiverCounts.hasOwnProperty(ev.cat)) byRiverCounts[ev.cat]++;
    }
  }
...
export function computeOuts(){
  ...
  const remaining = makeDeck().filter(c=>!PCalcState.getSelected().includes(cardId(c)));
  const targets=[CAT.PAIR,CAT.TWO,CAT.TRIPS,CAT.STRAIGHT,CAT.FLUSH,CAT.FULL,CAT.QUADS,CAT.STRAIGHT_FLUSH,CAT.ROYAL];
