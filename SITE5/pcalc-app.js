...
(function(g){
  ...
  function __rankCmp(evA, evB){
    const s = PC.cmpEval(evA, evB); // sentido desconhecido
  ...
  function calibrateCmpOnce(){
    ...
    const sf = [ {r:13,s:'s'},{r:12,s:'s'},{r:11,s:'s'},{r:10,s:'s'},{r:9,s:'s'} ];
    const hc = [ {r:14,s:'h'},{r:13,s:'d'},{r:7,s:'c'},{r:5,s:'s'},{r:3,s:'h'} ];
  ...
  function computeNutsPair(){
    ...
    // *** MUDANÇA: excluir apenas as cartas DO BOARD (nuts absoluto) ***
  ...
  function buildFullRankingPostflop(){
    ...
    // *** MUDANÇA: excluir apenas o BOARD para ranking absoluto ***
    const boardSet = new Set(board.map(cardId));
    const remaining = makeDeck().filter(c=>!boardSet.has(cardId(c)));
    ...
    // Garante entrada para a mão do herói (não muda ranking absoluto; só garante “label” presente)
    ...
    arr.sort((x,y)=> __rankCmp(x.ev,y.ev)); // melhor primeiro
    return arr.map(x=>({label:x.label, right:(CAT_NAME[x.ev.cat]||'')}));
  }

  // Mantém APIs de Top5 (usando rankings completos)
